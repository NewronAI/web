/* global React */
const { PageHero, Band, Head, FeatureGrid, FAQ, CTABand, Arrow, renderPage } = window;
  ...
  return (
    <div style={{ marginTop: 48, borderTop: "1px solid var(--line)" }}>
      {RELEASES.map(([date, title, desc]) =>
        <a key={title} href="v4.html#contact" style={{ display: "grid", gridTemplateColumns: "140px 1fr auto", gap: 28, alignItems: "start", padding: "24px 0", borderBottom: "1px solid var(--line)", textDecoration: "none", color: "var(--ink)" }}>
  ...
  return (<>
    ...
    <Band id="kit">
      ...
      <div style={{ marginTop: 48, display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
        {[["Logo & wordmark", "PNG + SVG · light and dark", "newron-logo.png"], ["Company boilerplate", "One-paragraph and one-line descriptions", "v4.html#contact"], ["Imagery", "Approved product and brand visuals", "v4.html#contact"]].map(([t, d, href]) =>
          <a key={t} className="card" href={href} style={{ padding: 28, textDecoration: "none", color: "var(--ink)", display: "block" }}>
