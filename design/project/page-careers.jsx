/* global React */
const { PageHero, Band, Head, SplitRows, Timeline, FAQ, CTABand, Arrow, renderPage } = window;
      ...
      {groups.map((g) =>
        <div key={g}>
          ...
          {ROLES.filter((r) => r[0] === g).map(([, title, loc]) =>
            <a key={title} href="v4.html#contact" style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: 24, alignItems: "center", padding: "20px 0", borderTop: "1px solid var(--line)", textDecoration: "none", color: "var(--ink)", transition: "padding 0.2s" }}
