/* global React, ReactDOM, TweaksPanel, TweakSection, TweakRadio, TweakSelect, TweakColor, TweakText, useTweaks */
...
const TWEAK_DEFAULTS = /*EDITMODE-BEGIN*/{
  ...
  "surface": "default",
  "density": "regular",
  "heroHeadline": "Intelligence, built for the institutions that can't get it wrong.",
  "heroSub": "Newron is the applied-AI partner to India's banks, NBFCs, insurers and state institutions — building production systems that underwrite faster, settle claims sooner, and serve citizens in their own language.",
  "ctaCopy": "See Newron on your data, in a week."
} /*EDITMODE-END*/;

/* DS brand violet primary, plus harmonious near-neighbours from the violet scale and a dark ink */
const ACCENT_OPTIONS = ["#6B4EE8", "#5638D4", "#8E73F0", "#432AA8", "#0E0B1F"];
...
function Mark({ size = 24 }) {
  return (
    <img src="newron-logo.png" alt="" width={size} height={size} style={{ display: "block" }} />
  );
}
  ...
  return (
    <header style={{
      ...
      <div className="shell" style={{
        ...
        alignItems: "center", gap: "clamp(16px, 3vw, 48px)"
      }}>
        <Wordmark />
        <nav className="nav-center" style={{ display: "flex", justifyContent: "center", gap: 2 }}>
          ...
          <a key={l} href={h} style={{
            ...
            padding: "8px 10px", fontSize: 13.5, fontWeight: 500, borderRadius: 4,
            whiteSpace: "nowrap"
          }}>{l}</a>
          )}
        </nav>
...
function Hero({ headline, sub }) {
  return (
    <section id="top" className="dotgrid-soft" style={{ paddingBlock: "calc(48px * var(--density)) calc(96px * var(--density))", position: "relative" }}>
      <div className="shell" style={{ position: "relative" }}>
        {/* Top eyebrow row */}
        <div className="hr-bot" style={{
          paddingBottom: 18, marginBottom: 72,
          display: "grid", gridTemplateColumns: "1fr 1fr 1fr",
          alignItems: "center", gap: 16
        }}>
          <div className="eyebrow">Newron · Bengaluru</div>
          <div className="eyebrow" style={{ textAlign: "center" }}>NVIDIA Inception Partner</div>
          <div className="eyebrow" style={{ textAlign: "right" }}>SOC 2(APPLIED) · ISO 27001</div>
        </div>
        ...
        <div style={{
          ...
          <h1 className="display" style={{
            ...
            <br />
            that <em className="italic" style={{ color: "var(--accent)" }}>can't get it&nbsp;wrong.</em>
          </h1>
        ...
        {/* Below the fold sub-row: index of contents */}
        <ContentsIndex />
      </div>
    </section>);
...
function ContentsIndex() {
  ...
  return (
    <div style={{ marginTop: 96, borderTop: "1px solid var(--line)" }}>
      {rows.map((r, i) =>
      <a key={r.n} href={r.href} style={{
        display: "grid",
        gridTemplateColumns: "100px minmax(0, 3fr) minmax(0, 5fr) auto",
        alignItems: "center",
        gap: 32,
        padding: "26px 0",
    ...
    <section id="lending" className="section">
      <div className="shell">
        ...
        <div style={{
          ...
          <CoverageCard
            label="Commercial"
            items={["Loan against property", "Overdraft", "Gold loan", "Equipment finance", "Revenue-based finance", "Line of credit"]} />
          ...
          <aside style={{ borderRight: "1px solid var(--line)", padding: 24, background: "var(--bg)" }}>
            ...
            <button
              ...
              style={{
                ...
                <div className="mono" style={{ fontSize: 10, letterSpacing: "0.1em", color: active === s.id ? "var(--accent)" : "var(--ink-muted)", marginBottom: 6 }}>
                  {s.tag}
                </div>
        ...
        {/* Outcomes — anchored to the marquee section */}
        <Outcomes />
      </div>
    </section>);
...
function CoverageCard({ label, items }) {
  return (
...
function ProductChrome({ title, children }) {
  return (
    <div style={{
      ...
      borderRadius: 6,
      overflow: "hidden",
      boxShadow: "0 30px 80px -40px rgba(0,0,0,0.18)"
    }}>
      <div style={{
...
function CAMScreen() {
  const sections = [
  ["01", "Borrower profile", "done"],
  ["02", "Business overview", "done"],
  ["03", "Financial summary", "done"],
  ["04", "Cash-flow analysis", "done"],
  ["05", "Bureau analysis", "done"],
  ["06", "Collateral", "active"],
  ["07", "Deviations", "queued"],
  ["08", "Recommendation", "queued"]];
    ...
    <div style={{ display: "grid", gridTemplateColumns: "240px 1fr 280px", minHeight: 460 }}>
      <div style={{ borderRight: "1px solid var(--line)", padding: 20 }}>
        ...
        <div key={n} style={{
          ...
          borderRadius: 3,
          fontSize: 13,
          color: s === "queued" ? "var(--ink-muted)" : "var(--ink)"
        }}>
            <span className="mono" style={{ fontSize: 10, color: "var(--ink-muted)" }}>{n}</span>
            <span>{l}</span>
            <span className="mono" style={{ fontSize: 9, letterSpacing: "0.1em", color: s === "done" ? "var(--green)" : s === "active" ? "var(--accent)" : "var(--ink-muted)" }}>
              {s === "done" ? "DONE" : s === "active" ? "WRITING" : "—"}
            </span>
          </div>
        )}
      </div>
      ...
      <div style={{ padding: 32 }}>
        ...
        <div style={{ fontFamily: "var(--font-display)", fontSize: 28, lineHeight: 1.05, marginBottom: 22 }}>
        ...
        <div style={{
          ...
          borderLeft: "2px solid var(--accent)", background: "var(--bg-2)",
      ...
      <div style={{ borderLeft: "1px solid var(--line)", padding: 20, background: "var(--bg)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {[
          ["Deviation", "DTI > 0.45 by 2pp", "amber", "Tier-2 override permitted"],
          ["Confirmed", "Property age within limit", "green", "EC clean, title singular"],
          ["Verify", "Q1→Q2 income variance", "slate", "Re-check queued"]].
          map(([k, v, c, s]) =>
  ...
  return (
    <div style={{ padding: 28, minHeight: 460 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 24 }}>
        {[
        ["Total inflow", "₹ 47.4L", "green"],
        ["Avg balance", "₹ 1.84L", "ink"],
        ["Bounce events", "0", "green"],
        ["Salary regularity", "11/12", "amber"]].
        map(([k, v, c]) =>
    ...
    <div style={{ padding: 28, display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 24, minHeight: 460 }}>
      ...
      <div>
        ...
        <div style={{ borderLeft: "1px solid var(--line)", paddingLeft: 16 }}>
          {[
          ["09:14", "Application received via LOS", "muted"],
          ["09:14", "KYC + bureau pulled", "muted"],
          ["09:17", "Statement analyser · 11 mo", "green"],
          ["09:19", "Video PD scheduled", "slate"],
          ["09:42", "Video PD complete", "green"],
          ["09:43", "CAM v1 generated", "accent"],
          ["09:44", "Deviation flagged · DTI", "amber"],
          ["—", "Awaiting officer review", "muted"]].
          map(([t, s, c], i) =>
  ...
  return (
    <div style={{ padding: 28, minHeight: 460 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 20 }}>
        <div style={{
          aspectRatio: "16/10",
          background: "linear-gradient(135deg, #2c2a24, #16140f)",
          borderRadius: 6,
          position: "relative",
          overflow: "hidden",
          border: "1px solid var(--line)"
        ...
        <div>
          ...
          <div className="card" style={{ padding: 14 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 12, fontSize: 12.5 }}>
              {[
              ["Boundary match", "0.94", "green"],
              ["Door no. visibility", "Confirmed", "green"],
              ["Plot extent (est.)", "1,840 sq ft", "ink"],
              ["Construction stage", "Completed", "green"],
              ["Encroachment risk", "None", "green"],
              ["Surroundings", "Residential", "ink-soft"]].
              map(([k, v, c]) =>
              <div key={k} style={{ display: "flex", justifyContent: "space-between" }}>
                  <span style={{ color: "var(--ink-soft)" }}>{k}</span>
                  <span className="mono" style={{ fontSize: 11.5, color: c === "ink" || c === "ink-soft" ? `var(--${c})` : `var(--${c})` }}>{v}</span>
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "240px 1fr", minHeight: 460 }}>
      ...
      <div style={{ padding: 22, display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <Msg from="Pratiksha · Credit Officer" right>
          Ticket is ₹ 38L, Bengaluru — Tier-1?
        </Msg>
        <Msg from="Newron" accent>
...
function Msg({ from, right, accent, children }) {
  return (
    <div style={{ alignSelf: right ? "flex-end" : "flex-start", maxWidth: "82%" }}>
      ...
      <div style={{
        padding: "12px 14px",
        background: right ? "var(--bg-2)" : "var(--bg)",
        border: "1px solid var(--line)",
  ...
  return (
    <div style={{ marginTop: 56, display: "grid", gridTemplateColumns: "minmax(0, 2fr) repeat(3, 1fr)", gap: 0, borderTop: "1px solid var(--line)" }}>
      ...
      <div key={m.k} style={{ padding: 24, borderLeft: "1px solid var(--line)" }}>
          ...
          <div className="mono" style={{ fontSize: 11, color: "var(--ink-muted)", letterSpacing: "0.08em", marginTop: 14 }}>
            {m.k.toUpperCase()}
          </div>
        </div>
      )}
            ...
            <ProductChrome title="Claims · Policy CLP-44829-1">
              <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
                  {[
                  ["Policy fit", "0.98", "green"],
                  ["Denial risk", "0.18", "amber"],
                  ["Doc completeness", "11/11", "green"]].
                  map(([k, v, c]) =>
                ...
                <div style={{ borderLeft: "2px solid var(--accent)", padding: 12, background: "var(--bg-2)", borderRadius: "0 4px 4px 0" }}>
                  ...
                  <div style={{ fontSize: 13.5, lineHeight: 1.5 }}>
                    Claim is well within sub-limit. Recommend filing today via Star Allied TPA;
                    historical SLA is 4.2 working days.
                  </div>
                </div>
...
function ClaimStep({ n, title, desc, meta, metaColor, last }) {
  return (
    <div style={{
      paddingBlock: 22,
      borderBottom: last ? "none" : "1px solid var(--line)",
      display: "grid", gridTemplateColumns: "32px 1fr auto", gap: 16, alignItems: "start"
    }}>
...
function Governance() {
  return (
    <section id="governance" className="section" style={{ background: "var(--inverse-bg)", color: "var(--inverse-ink)" }}>
      <div className="shell">
        <SectionHead
          inverse
          tag="03"
        ...
        <div style={{ marginTop: 56, display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(0, 1.3fr)", gap: 64, alignItems: "start" }}>
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0 }}>
              ...
              ["Regional TTS", "Natural voices, low latency"],
              ...
              <div key={k} style={{
                ...
                borderRight: i % 2 === 0 ? "1px solid var(--inverse-line)" : "none",
          ...
          <div style={{
            ...
            <Bubble who="Citizen · Kannada audio">
              <span style={{ fontFamily: "var(--font-display)", fontSize: 18, color: "var(--inverse-ink)" }}>
            ...
            <Bubble who="Draft response · Kannada" accent>
              <span style={{ fontFamily: "var(--font-display)", fontSize: 16, color: "var(--inverse-ink)" }}>
                ನಿಮ್ಮ ರೇಶನ್ ಕಾರ್ಡ್ ಮಾರ್ಚ್ 12 ರಂದು ಮುದ್ರಿತವಾಗಿದೆ…
              </span>
...
function Bubble({ who, accent, children }) {
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      <div style={{
        border: "1px solid var(--inverse-line)",
        borderLeft: accent ? "2px solid var(--accent)" : "1px solid var(--inverse-line)",
        padding: "10px 12px",
        borderRadius: 4,
        fontSize: 13,
        lineHeight: 1.55,
    ...
    <section id="customers" className="section" style={{ borderTop: "1px solid var(--line)", background: "var(--bg-2)" }}>
      <div className="shell">
        <SectionHead
          tag="05"
          eyebrow="Customers"
          title={<>In production at banks, NBFCs<br />and state institutions.</>} />
        ...
        <div style={{ marginTop: 56, display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(0, 1.6fr)", gap: 56, alignItems: "start" }}>
          <div>
            ...
            <blockquote style={{ margin: 0, fontFamily: "var(--font-display)", fontSize: "clamp(26px, 2.6vw, 36px)", lineHeight: 1.2, letterSpacing: "-0.012em" }}>
              ...
              Our credit officers stopped reformatting Excel and went back to actually underwriting."
            </blockquote>
          ...
          <div style={{
            ...
            {[
            ["Kotak", "Bank"],
            ["Satya MicroCapital", "NBFC"],
            ["Govt. of Karnataka", "State"],
            ["PhonePe", "Fintech"],
            ["Akmedeus", "Health"],
            ["Truker", "Mobility"],
            ["TCI", "Logistics"],
    ...
    <section id="company" className="section">
      <div className="shell">
        <SectionHead
          tag="06"
          eyebrow="Why Newron"
          title={<>A platform built for the way<br /><em className="italic" style={{ color: "var(--accent)" }}>regulated industries</em> actually buy AI.</>} />
...
function CTA({ headline }) {
  ...
  return (
    <section id="contact" className="section" style={{ background: "var(--inverse-bg)", color: "var(--inverse-ink)" }}>
      <div className="shell" style={{ display: "grid", gridTemplateColumns: "minmax(0, 1.3fr) minmax(0, 1fr)", gap: 80, alignItems: "end" }}>
        <div>
          ...
          <p style={{ fontSize: 16, color: "var(--inverse-ink-soft)", maxWidth: 520, marginTop: 28, lineHeight: 1.5 }}>
            We'll spin up a sandboxed instance against a slice of your data, deliver a working pilot,
            and give your team the eval numbers to make the decision.
          </p>
        </div>
        ...
        <div style={{ border: "1px solid var(--inverse-line)", padding: 28, borderRadius: 6, background: "var(--inverse-bg-2)" }}>
            ...
            <div className="mono" style={{ fontSize: 11, color: "var(--inverse-ink-soft)", letterSpacing: "0.08em", marginTop: 12 }}>
              TICKET · NWR-2026-{Math.floor(Math.random() * 90000 + 10000)}
            </div>
          </div> :

        <form onSubmit={(e) => {e.preventDefault();setSubmitted(true);}} style={{
          border: "1px solid var(--inverse-line)", padding: 24, borderRadius: 6, background: "var(--inverse-bg-2)",
          display: "flex", flexDirection: "column", gap: 14
            ...
            <button type="submit" className="btn" style={{ background: "var(--accent)", color: "#fff", marginTop: 6, justifyContent: "space-between" }}>
  ...
  return (
    <footer style={{ background: "var(--bg)", borderTop: "1px solid var(--line)", paddingBlock: 80 }}>
      <div className="shell">
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 2fr) repeat(4, minmax(0, 1fr))", gap: 32 }}>
          <div>
            ...
            <div style={{ marginTop: 18 }}>
              <span className="tag">NVIDIA INCEPTION</span>
            </div>
          </div>
          {cols.map((c) =>
          <div key={c.h}>
        ...
        <div style={{ borderTop: "1px solid var(--line)", marginTop: 64, paddingTop: 22, display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--ink-muted)", fontFamily: "var(--font-mono)" }}>
...
// ───────────────────────────────────────────────── shared SectionHead
function SectionHead({ tag, eyebrow, title, kicker, inverse }) {
  return (
    <header style={{
      display: "grid",
      gridTemplateColumns: "80px minmax(0, 1.4fr) minmax(0, 1fr)",
      gap: 48,
      alignItems: "start",
      borderTop: `1px solid ${inverse ? "var(--inverse-line)" : "var(--line)"}`,
      paddingTop: 28
    }}>
      <div className="mono" style={{ fontSize: 12, color: inverse ? "var(--inverse-ink-soft)" : "var(--ink-muted)", letterSpacing: "0.1em", paddingTop: 6 }}>
        {tag}
      </div>
      <div>
        <div className="eyebrow" style={{ marginBottom: 18, color: inverse ? "var(--inverse-ink-soft)" : "var(--ink-muted)" }}>{eyebrow}</div>
        <h2 className="display" style={{
          margin: 0,
          fontSize: "clamp(48px, 6.4vw, 88px)",
          lineHeight: 0.98,
          color: inverse ? "var(--inverse-ink)" : "var(--ink)"
        }}>
          {title}
        </h2>
      </div>
      <div>
        {kicker &&
        <p style={{
          margin: 0, marginTop: 38, fontSize: 16, lineHeight: 1.55,
          color: inverse ? "var(--inverse-ink-soft)" : "var(--ink-soft)",
          maxWidth: 380
        }}>
            {kicker}
          </p>
...
// blink keyframes
if (!document.querySelector("#blink-kf-v2")) {
...
function App() {
  ...
  React.useEffect(() => {
    ...
    if (t.surface && t.surface !== "default") r.setAttribute("data-surface", t.surface);else
    r.removeAttribute("data-surface");
    r.style.setProperty("--accent", t.accent);
    r.style.setProperty("--accent-soft", `color-mix(in oklab, ${t.accent} 10%, transparent)`);
  }, [t]);

  return (
    <>
      <Nav />
      <Hero headline={t.heroHeadline} sub={t.heroSub} />
      <LogoMarquee />
      <Lending />
      <Insurance />
      <Governance />
      ...
      <TweaksPanel>
        ...
        <TweakSelect label="Surface" value={t.surface || "default"} options={["default", "paper", "mist", "violet", "ink"]} onChange={(v) => setTweak("surface", v)} />
        ...
        <TweakText label="CTA headline" value={t.ctaCopy} onChange={(v) => setTweak("ctaCopy", v)} />
      </TweaksPanel>
    </>);
